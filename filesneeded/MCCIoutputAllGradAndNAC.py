...
#modified to get gradient for all states
#also  NAC between all states i>j
...
def mySCI_Grad_Nac(x): 
	...
	for i in range(states):
		myCommandString="mv Tran2RDM_state"+str(i+1)+"state"+str(i+1)+" NonZero2RDM_MO_Coeffs"
		#print(myCommandString)
		os.system(myCommandString)
		os.system(SCIgradLocation) 
		myCommandString="mv SCIgradient.txt SCIgradient_state"+str(i+1)+".txt"
		os.system(myCommandString)
...
mol = gto.Mole()
mol.atom='''C 0.0 0.0 0.00000000; O 0.0 0.0 1.0'''  # 
mol.basis='STO-3G'
mol.unit='B' #Use bohr
mol.symmetry = True
...
twoRDMcommand='mpirun -n 1 '+mybin+'transition2RDM/mcci'
NACcommand=mybin+'Output_NAC'   

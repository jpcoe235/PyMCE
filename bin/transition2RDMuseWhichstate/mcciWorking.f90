...
! Adapt to give transition density matrix when using state averaging -- done
! added two ground state diags before excited for stability -- done still crashes on some runs for helium in v small basis

!change 1RDM coeffs to standard definition -- done  
program mcci
  use commonarrays
  use dyn_par
...
!for density
  ...
  integer l,k,spinl,spink
  double precision ep 
  double precision, allocatable :: DenCoeffs(:,:)
!for 2RDM
! only if ntotal>2 as need 3 electrons at least
 double precision, allocatable :: TwoRDMCoeffs(:,:,:,:)
 double precision eg,dtemp
 integer l2,k2,itemp
integer i1,i2
...
!!!!!!!!!!!!!!!!!!!!!density 1RDM when just looking at one state (no state averaging)
if(me.eq.0) THEN
if(use_sds) THEN
PRINT *,nbft,ntotal
!check norm
...
do jci=1,length
 ...
 dencoeffs(l,k)=dencoeffs(l,k)+(ep*c(ici)*c(jci))
    ...
    ELSE !ndiff=0
      ...
      dencoeffs(j,j)=dencoeffs(j,j)+(c(ici)*c(jci))
      end do
      END IF
...
OPEN(UNIT=15,FILE='Den_MO_Coeffs')
do i=1,nbft
do j=i,nbft
if (dencoeffs(i,j).eq.0.0D0) cycle
...
dtemp=0.0D0
do i=1,nbft
dtemp=dtemp+dencoeffs(i,i)
end do

PRINT *,'Sum diag 1RDM',dtemp
...
!!!!!!!!!2RDM  ! include this in 1RDM calc to increase efficiency later
if((me.eq.0).AND.use_sds.AND.(ntotal.gt.2)) THEN
PRINT *, 'In 2RDM'

ALLOCATE(TwoRDMCoeffs(2*nbft,2*nbft,2*nbft,2*nbft)) ! now need number of spin orbitals for each dimension
...
!        PRINT *,l,l2,k,k2
 dtemp=TwoRDMCoeffs(l,l2,k,k2)
  dtemp=dtemp+(c(ici)*c(jci)*ep*eg)
    TwoRDMCoeffs(l,l2,k,k2)=dtemp

       end if  !ndiff=2

         if(ndiff.eq.1) THEN ! double check this derivation
       ...
       if(i.eq.idiff1) cycle !can't be one of the single differences
                 ...
                 !initial order of geminals from reordering dets depends on whethere the non-difference is before or after
       if(i.lt.idiff1) THEN  ! order in Slater determinant 
        l=list(1,i)
        l2=list(1,idiff1)
        k=list(2,i)
        k2=list(2,idiff1)
        ...
        END IF
        eg=1.0D0
       if(l.gt.l2) THEN  !then put geminal in standard order with sign changes if necessary
       itemp=l
        l=l2
        l2=itemp
         eg=-1.0D0*eg
...
!        PRINT *,l,l2,k,k2
 dtemp=TwoRDMCoeffs(l,l2,k,k2)
  dtemp=dtemp+(c(ici)*c(jci)*ep*eg)
   ...
   !now need to do no differences
    if(ndiff.eq.0) THEN

      do i1=1,ntotal  ! all possible geminals -- all possible pairs from orbital list. Same geminal on left and right as determinants are the same
      do i2=i1,ntotal
      if(i1.eq.i2) cycle
        ...
        l2=list(1,i2)
         k=l
       k2=l2
        !note left sign change will cancel right so don't need to use eg here but we we will for consistency
        eg=1.0D0
       if(l.gt.l2) THEN  !then put geminal in standard order with sign changes if necessary
       itemp=l
        l=l2
...
end do !loop over jci
end do ! loop over ici

OPEN(UNIT=15,FILE='2RDM_MO_CoeffsGreaterThan0.4')
do l=1,2*nbft
do l2=l,2*nbft
if (l.eq.l2) cycle
do k=1,2*nbft
...
if (ABS(TwoRDMCoeffs(l,l2,k,k2)).lt.0.4D0) cycle
WRITE(15,*) l,l2,k,k2,TwoRDMCoeffs(l,l2,k,k2)
end do
end do
end do
end do
CLOSE(15)
DEALLOCATE(TwoRDMCoeffs)
end if

!!!!!!!!!!!!!!!!!!


!Deallocate ctemp if running SA-MCCI and output energies and vectors for each state
if(sa_mcci) THEN
  if(me.eq.0) THEN
...
do is=1,ieig-1
            ...
            open(40,file=f1,form='formatted')
            do i=1,length
               do n=1,nword
                  if(n.eq.1) then
write(40,'(i6,2x,e24.17,2x,i11,2x,i11)')&
i,ctemp(i,is)/dsqrt(dnorm),icij(1,n,i),icij(2,n,i)
                  else
write(40,'(33x,i11,2x,i11)') icij(1,n,i),icij(2,n,i)
                  endif
               enddo
            enddo
            close(40)
...
!!!! Write out expectation values for energies

OPEN(UNIT=15,FILE='E_expect')
 call energy(length,eval,dnorm)
WRITE(15,*) ieig,eval+ecore

 ctemp(:,ieig)=c(:)
do is=1,ieig-1
 c(:)=ctemp(:,is)
    call energy(length,eval,dnorm)
WRITE(15,*) is,eval+ecore
end do
 CLOSE(15)
!!!!!
END IF

...
!     This makes them easy to pass to the parts of mcci.f90 that need them - just follow
      ...
      integer conv_ltarget    ! minimum length for convergence check 
      ...
      logical run_pt2         ! Run mccipt2 after mcci calculation only for CSFs
      logical sa_mcci         ! Sum absolute coefficients over first ieig states for pruning
      logical use_sds         ! Use Slater Determinants instead of CSFs
      logical spin_print      ! Calculate and print expectation of S^2 at each step when using SDs and sa_mcci
end module dyn_par

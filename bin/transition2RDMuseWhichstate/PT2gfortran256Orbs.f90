...
!does not work with active space at the moment

! designed for 97 to 192 orbs but will work with fewer although maybe slower 
!This should be 2*128=256 orbitals as a maxmum so edited with firstwrd to allow up to 256
!Seems to work ok with up to nword=5 (up to 160 basis functions)

! try to put active space in
...
!         do jj=1,nactive
 ...
 !       PRINT *,nbft-nactive
         ...
         maxpt=1000*maxc !add estimate based on length*number of single doubles
         contribthresh=cmin
...
!!!!!!!!!!!!!!!!!
           ...
           totalI=0
            ...
            ALLOCATE(Ipt(2,iword,maxpt))
              Ipt=0
              do i=1,ilength
              molist=0 ! zero all MOs then add back active space then remove occupied that are not frozen
...
!        PRINT *,(molist(jj,ispin),jj=1,nbft-delmo(ispin))
!             PRINT *,'*****'   
!        PRINT *,(orblist(jj,ispin),jj=1,n_alpha-delorb(ispin))
!        PRINT *,'*****'
!        READ *,jj
! for each spin check swap with each unoccupied mo
! put symmetry check here     
...
! compare irpod+1 with desired symmetry
           if((isym1).ne.isym2) cycle
!           PRINT *,molist(mo1,ispin),orblist(orb1,ispin)
!           PRINT *,isym1,isym2,irprod
!           PRINT *,symreq,'Is requested sym'
...
!         if( btest(Ipt(ispin,n1,il1),j1) ) THEN
!         PRINT *,j+1
!         END IF
!         end do
...
!!!!!!!!!!!!!! Sorting
         ...
         list=0
! switch to integer*16 c swap Ipt in sorting subroutine at the moment
         firstwrd=nword  ! put up to first 4 in the 16 bit array
         if(firstwrd.gt.4) firstwrd=4
...
list(i1)=ibset(list(i1),ibit+(32*(n-5)))
         end do
         end do       
         end do
...
!        do i=1,1000
!        PRINT *,'******'
!        PRINT *,(Ipt(1,j,i),j=1,nword)
!         PRINT *,(Ipt(2,j,i),j=1,nword)
!        end do
...
! Slower but ref space
! is small in comparision to PT space
         do j1=1,totalI
...
! mcci vector is divided by SQRT dnorm single trial PT function SQRT by s_overlap                   
         contrib=((ptnum**2)) 
...
ABS(dnorm*contribthresh*((Ek*s_overlap)-e))) THEN  !if removing division then swap gt to lt
         ...
         contrib=contrib/(dnorm*((s_overlap*Ek)-e))  ! with s_overlap for e PT est was much too high

subroutine h_s_sparse_sd(length,llast)
  ...
  do ici = llast+1, length
     call muHnu_sd(ici,ici,eij,n_2p,s_overlap)
!     cnorm(ici) = dsqrt( dabs(ck(i_sx2,n_2p,0)) )  !Commented out for SDs
     h(ici) = eij
     s(ici) = s_overlap    !Changed to s_overlap
  enddo
  ijh(1) = length + 2
  ijs(1) = length + 2
  ...
  do ici = llast+1, length
     do jci = 1, ici-1
        ...
        call muHnu_sd(ici,jci,eij,n_2p,s_overlap)

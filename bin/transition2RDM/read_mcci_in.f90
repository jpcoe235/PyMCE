subroutine read_mcci_in(iword,maxc,maxocc,maxbfs,               &
  ...
  use dyn_par,      only: maxtry, lmin,nbyte,int_bits,npfull,   &
                          ...
                          run_pt2,sa_mcci,conv_ltarget,use_sds,   &
                          spin_print
  ...
  do
     ...
     read(value_string,*) run_pt2
       ...
       case ('sa_mcci')
       read(value_string,*) sa_mcci
            
       case ('use_sds')
       read(value_string,*)  use_sds
   
         case ('spin_print')
       read(value_string,*)  spin_print

        case ('conv_ltarget')
        read(value_string,*) conv_ltarget
        call non_negative_integer(conv_ltarget,'conv_ltarget')

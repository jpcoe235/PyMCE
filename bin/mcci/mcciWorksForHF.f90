...
!put 1RDM_SD in a subroutine - done

! get 2RDM in standard definition for single HF SD  ! this works for elements adn energy
...
double precision, allocatable :: SpinFree2RDM(:,:,:,:)
 ...
 integer l2,k2,itemp,spinl2,spink2
integer i1,i2
...
!!!!!!!! testing of 1RDM to give energy (delete later)
...
if (ABS(SpinFree2RDM(l,l2,k,k2)).lt.0.1D0) cycle
...
!Check summation 'trace'

dtemp=0.0D0
do l=1,nbft
do k=1,nbft

dtemp=dtemp+SpinFree2RDM(l,k,k,l)
end do
end do

PRINT *,'like trace 2RDM',dtemp
...
dtemp=dtemp+(SpinFree2RDM(iorb,jorb,lorb,korb)*e2ints(ikjl)) 
end do
end do
end do
end do

PRINT *,'2RDM energy contribution',0.5D0*dtemp ! works for HF now remember we have 0.5 in the energy term not density matrix

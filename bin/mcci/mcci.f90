...
! get 2RDM in standard definition for single HF SD -- done works

!add two differences -- done won't know if right for FCI energy until added one difference also

!add one difference -- done v.slight difference with FCI... Let's turn off 2 difference to see if we can isolate issue in one difference

!found typo so one difference seems fine now 
!turn two differences back on in slater2 and 2RDM
!Now look at getting two differences to work
!Great this gives FCI energy agreement now

!Can we use 2RDM to give 1RDM energy contribution -- yes this is done now
!Turn off 1RDM for the moment as we are calculating 2RDM and this includes 1RDM

!Put 2RDM_SD as subroutine -- done

!TO DO update the hpc7 code folder her and on hpc7 as it does not use the 2RDM subroutine
program mcci
  use commonarrays
  use dyn_par
  USE MPI
...
if(me.eq.0) THEN
if(use_sds) THEN
if(1.eq.2) THEN ! switch this offf
